# Milestone v1.1: DDD sem Mudar a Arquitetura
...
Módulo pedagógico "Fase 0": demonstrar DDD aplicado dentro da arquitetura tradicional Controller→Service→Repository, sem introduzir Clean Architecture, Hexagonal ou padrões avançados. O desenvolvedor estuda dois módulos Maven completos com o mesmo domínio (Matrícula Escolar) resolvido de formas opostas — mesmas requisições HTTP, mesmos dados no banco, decisões de design completamente diferentes.
...
|-------|----------------|--------|-----------|
| 5. Diagnostico — Codigo com Anti-padroes | 6/6 | Complete | 2026-06-22 |
| 6. Refatoracao DDD na Arquitetura Tradicional | 5/5 | Complete | 2026-06-22 |
| 7. Analise Final e Balanco Didatico | 1/1 | Complete | 2026-06-22 |
...
- Multi-module Maven adotado (exceção pedagógica): permite dois sistemas completos lado a lado com o mesmo domínio, evidenciando as diferenças de design sem abstração
- Módulo `erp-matricula-camadas` criado do zero com anti-padrões intencionais — código "errado" documentado com comentários `ANTI-PADRAO: DIAG-XX`
- Comentários REFD inline inseridos nos 3 pivots Java do módulo DDD (`Matricula.java`, `MatricularAlunoUseCase.java`, `MatriculaRepositorio.java`) sem alterar comportamento
- `12-analise-final.md` entregue com dados concretos do projeto (42 arquivos, 3.514 LOC, 3 invariantes, 227 linhas do God Service, 5 conceitos DDD)

**Issues Deferred:**

- Phase 05 HUMAN-UAT: 2 cenários de verificação pedagógica (discrepância de trail + DisciplinaServiceImpl HTTP reachability) — julgamento editorial, não bug de código
- Phase 07 UAT: 3 verificações de capacidade argumentativa do leitor — requerem leitura humana do documento
- Phase 05/07 VERIFICATION: status `human_needed` — conteúdo 6/7 e 5/5 truths verificadas automaticamente; pendências são julgamentos pedagógicos

**Technical Debt Incurred:**

- DIAG-01 a DIAG-06 no REQUIREMENTS.md nunca foram marcados como `[x]` — validação ocorreu via código e documentação criados, mas o REQUIREMENTS foi atualizado parcialmente
- `DisciplinaServiceImpl` não está exposto via HTTP — duplicação (DIAG-04) é visível por leitura, mas não executável pela rota `/disciplinas/extra`
