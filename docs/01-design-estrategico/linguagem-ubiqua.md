...
|-------|-----------|---------|----------|
| Aluno | Pessoa física matriculável. No BC Matrícula, o que importa é se pode se matricular (está ativo? tem impedimentos?). Referenciado apenas por `AlunoId` — o BC Matrícula não carrega os dados completos do aluno. | Matrícula | `StudentEntity`, `Student`, `Usuario` |
